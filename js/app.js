...
function init(){
 ...
 const savedBg=localStorage.getItem('kow_bg_v4');if(savedBg)document.body.style.backgroundImage=`linear-gradient(rgba(0,0,0,.48),rgba(0,0,0,.78)),url('${savedBg}')`;
 ...
 $('backgroundPicker').onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{localStorage.setItem('kow_bg_v4',r.result);document.body.style.backgroundImage=`linear-gradient(rgba(0,0,0,.48),rgba(0,0,0,.78)),url('${r.result}')`};r.readAsDataURL(f)};
 $('resetAppearance').onclick=()=>{localStorage.removeItem('kow_app_name_v4');localStorage.removeItem('kow_bg_v4');location.reload()};
